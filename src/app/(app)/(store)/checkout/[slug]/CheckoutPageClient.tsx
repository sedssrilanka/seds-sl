...
export function CheckoutPageClient({ product }: CheckoutPageClientProps) {
  ...
  const validateField = (field: string, val: string) => {
    ...
    if (field === "name") {
      ...
      else if (val.trim().replace(/\D/g, "").length < 9)
        err = "Enter a valid phone number (min 9 digits)";
    } else if (field === "city") {
      ...
      if (!val.trim()) err = "Delivery address is required";
        ...
        err = "Please enter your full street delivery address";
    }
    return err;
  };

  const handleFieldChange = (
    field: "name" | "email" | "phone" | "city" | "address",
    val: string,
  ) => {
  ...
  const paymentReference =
    `${name || "Order"} - ${phone || product.title}`.trim();
  ...
  const buildTallyUrl = () => {
    ...
    const params = new URLSearchParams({
      ...
      product_name: product.isPreOrder
            ...
            <div className="border border-border/60 bg-background p-6 sm:p-8 space-y-6">
              ...
              <div className="flex gap-4 items-center">
                <div className="relative w-20 h-20 bg-muted/20 border border-border/40 shrink-0 overflow-hidden">
                  <Image
                    src={
                      product.image || "/images/products/tshit-2026-front.png"
                    }
                    alt={product.title}
                    fill
              ...
              <div className="pt-4 border-t border-border/60 space-y-2.5 text-xs font-mono">
                ...
                {isBuy4Get1 && freeItems > 0 && (
                  <div className="text-[11px] text-primary font-bold">
                    🎁 Deal applied: {freeItems} free band
                    {freeItems > 1 ? "s" : ""} included!
                  </div>
                )}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-muted-foreground">
                  <span>Island-Wide Courier Delivery:</span>
                  <span className="text-foreground font-semibold">
                    Rs. {deliveryFee.toLocaleString()} LKR
                  </span>
                </div>
              ...
              <div className="p-3.5 border border-primary/20 bg-primary/5 text-xs text-muted-foreground">
                ...
                <p>
                  100% of merchandise proceeds directly fund university space
                  rocketry & CanSat engineering in Sri Lanka.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="border border-border/60 bg-background p-4 sm:p-6 md:p-8">
                  ...
                  <div className="pt-4 border-t border-border/60">
                    ...
                    {!isFormValid && (
                      <p className="text-center text-[11px] text-muted-foreground mt-2 font-mono">
                        Please fill all required fields above to proceed to bank
                        details.
                      </p>
                    )}
                  </div>
              ...
              {step === 2 && (
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Transfer the order amount directly to our official student
                      club bank account.
                    </p>
                  </div>
                    ...
                    <div className="space-y-3 text-xs font-mono">
                      ...
                      <div className="flex items-center justify-between p-3.5 bg-background border border-border/40 gap-2">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            copyToClipboard("1000889944", "accountNumber")
                          }
                      ...
                      <div className="flex items-center justify-between p-3.5 bg-background border border-border/40 gap-2">
                        <div className="min-w-0 flex-1 mr-2">
                          ...
                          <span className="text-foreground font-semibold text-xs truncate block">
                            {paymentReference}
                          </span>
                        </div>
                        <Button
                  ...
                  <div className="border border-border/60 bg-background overflow-hidden">
                    ...
                    >
                      <span className="font-mono uppercase font-bold text-xs text-foreground flex items-center gap-2">
                        ...
                        Step-by-Step Payment Instructions & FAQs
                      </span>
                      <ChevronDown
                    ...
                    {isFaqOpen && (
                      <div className="p-4 pt-2 border-t border-border/40 text-xs space-y-3 bg-muted/5 text-muted-foreground leading-relaxed">
                        <ol className="list-decimal list-inside space-y-2">
                          <li>
                            <strong>Online Banking / App:</strong> Open your
                            banking app (e.g. ComBank Q+, Flash, BOC Smart, HNB
                            Digital, FriMi, or any bank app).
                          </li>
                          <li>
                            <strong>Transfer Details:</strong> Enter Account No.{" "}
                          ...
                          <li>
                            <strong>Amount:</strong> Enter the exact amount{" "}
                          ...
                          <li>
                            <strong>Payment Remarks:</strong> Add your name and
                            phone:{" "}
                          ...
                          <li>
                            <strong>Save Proof:</strong> Download the PDF
                            receipt or take a clear screenshot of the successful
                            transfer.
                          </li>
                          <li>
                            <strong>CDM / Cash Deposit:</strong> You can also
                            deposit directly at any Commercial Bank CDM machine
                            island-wide.
                          </li>
                        </ol>
                      </div>
                  ...
                  <div className="pt-4 border-t border-border/60 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
                    ...
                    >
                      <ArrowLeft className="w-3.5 h-3.5 mr-1.5" /> Back to
                      Delivery Details
                    </Button>
                    <Button
                      onClick={() => setStep(3)}
              ...
              {step === 3 && (
                <div className="space-y-6">
                  <div>
                    <h2 className="text-2xl font-bold text-foreground">
                      3. Upload Transfer Slip
                    </h2>
                    <p className="text-xs text-muted-foreground mt-1">
                      Attach your deposit slip screenshot or photo to finalize
                      your order.
                    </p>
                  </div>
                  ...
                  <div className="pt-2 flex flex-col-reverse sm:flex-row items-center justify-between gap-3 sm:gap-0">
                    <Button
                      ...
                      className="text-muted-foreground hover:text-foreground text-xs cursor-pointer w-full sm:w-auto"
                    >
                      <ArrowLeft className="w-3.5 h-3.5 mr-1.5" /> Back to Bank
                      Details
                    </Button>
